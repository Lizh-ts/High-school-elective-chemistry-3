...
## 動機
在化學課時老師上到酸鹼滴定，老師問我是否能寫出一個程式，計算酸鹼滴定的pH值並繪圖，像簡報上呈現的pH滴定圖。
...
## 心得
經過這次我發現，原來講義也會犯這種錯誤，可能是沒有給教育部審查，難怪數學老師會建議我們複習時要看課本內容(雖然他很嫌棄108課綱的編排)。這個程式我也分享給化學老師，讓之後的實驗如果要對齊或比較時可以使用這個程式的結果；同時我也發現Python在科學計算的方便之處--不用強制定義型別，和強大的數值計算庫，使我在短時間內就處理好這個專案。
